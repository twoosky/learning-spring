...
* JPA 사용 방법
  * Entity 설정
    * `@Entity`: 해당하는 클래스 이름의 데이터베이스 테이블을 생성하고, 필드의 정보를 테이블 생성에 필요한 컬럼 정보로 사용
    * 해당 필드 위에 `@ID` 어노테이션을 사용해 기본키 값으로 설정 가능하고, `@GeneratedValue` 어노테이션을 통해 키 값 자동생성 설정 가능
  * 초기 데이터 설정
    * `resources/`에 확장자명이 `.sql`인 파일 생성
    * `resources/`안의 `.sql` 파일을 스프링 부트가 처음 로딩할 때 초기 데이터로 생성한다.
    * 해당 파일에서 sql 명령어를 통해 데이터 삽입 가능
  * `JpaRepository<모델 이름, 기본 키 타입>`를 상속받은 레포지토리 인터페이스를 생성하여 해당 엔티티에 대한 CRUD 처리 가능, sql 명령 없이 디비 관련 작업 수행 가능
  * 테이블 간의 연관 관계가 있을 때 연관 관계 매핑 어노테이션 사용
    * 다대일 [N:1]: `@ManyToOne`
    * 일대다 [1:N]: `@OneToMany`
    * 일대일 [1:1]: `@OneToOne`
    * 다대다 [N:M]: `@ManyToMany`
...
RESTful API Design
===
* api를 사용하는 소비자 관점에서 설계
* 각 리소스 별로 적절한 HTTP 메소드 제공
  * GET: 조회
  * POST: 생성
  * PUT: 수정
  * DELETE: 삭제
* 적절한 상태 코드를 결정해 반환해야함
* 토큰 발급/스프링 시큐리티/헤더 값 등을 통해 URI에 중요한 정보 포함 제어
* 복수 형태의 URI 사용, 특정한 리소스를 지정하는 경우 다음 depth의 URI로 표현
* 리소스에 대한 정보는 명사 형태로 표시
* 일관된 접근 end point를 사용
  * 단순한 `/search`메소드를 설계하고 파라미터/헤더 값을 이용해 여러 종류의 요청 처리가 될 수 있도록 설계 권장
